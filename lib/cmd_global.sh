#!/usr/bin/env bash
# ccws global [<name> | --unset]
# Manage the ~/.ccws/global file (user-default workspace).

# shellcheck disable=SC1091
_libdir="$(dirname "${BASH_SOURCE[0]}")"
[[ -z "${CCWS_COMMON_LOADED:-}" ]] && source "$_libdir/common.sh"
[[ -z "${CCWS_SCOPE_LOADED:-}"  ]] && source "$_libdir/scope.sh"

ccws_cmd_global() {
    local globalfile
    globalfile=$(ccws_scope_global_file)

    if [[ $# -eq 0 ]]; then
        if [[ -f "$globalfile" ]]; then
            ccws_scope_read_file "$globalfile" || {
                ccws_log_error "$globalfile is empty"
                return 1
            }
            return 0
        fi
        ccws_log_error "no global workspace set"
        return 1
    fi

    case "$1" in
        --unset|-u)
            if [[ -f "$globalfile" ]]; then
                rm -f "$globalfile"
                ccws_log_ok "removed global workspace"
                return 0
            fi
            ccws_log_warn "no global workspace set"
            return 0
            ;;
        -*)
            ccws_log_error "unknown flag: $1"
            return 2
            ;;
    esac

    local name="$1"
    ccws_validate_name "$name" || return 2

    local ws
    ws=$(ccws_ws_dir "$name")
    if [[ ! -d "$ws" ]]; then
        ccws_log_error "workspace '$name' does not exist (use 'ccws add $name' first)"
        return 1
    fi

    mkdir -p "$(dirname "$globalfile")"
    printf '%s\n' "$name" > "$globalfile"
    ccws_log_ok "set global workspace to '$name'"
}

export CCWS_CMD_GLOBAL_LOADED=1
